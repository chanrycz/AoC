...
var pairs = data.map(pairStr => pairStr.map(str => JSON.parse(str)));

// Part 1
var pairsInRightOrder = [];
...
var total = pairsInRightOrder.reduce((a, b) => a + b, 0)
...
var allPackets = [dividerPacket1, dividerPacket2];
pairs.forEach(([left, right]) => allPackets.push(left, right));
allPackets.sort((a, b) => compare(b, a));

var decoderKey = (1 + allPackets.findIndex(packet => compare(packet, dividerPacket1) === 0)) * (1 + allPackets.findIndex(packet => compare(packet, dividerPacket2) === 0))
...
function compare(left, right) {
    ...
    if (Array.isArray(left) || Array.isArray(right)) {
        ...
        return arrCompare(leftArr, rightArr);
    }
...
function arrCompare(left, right) {
    var maxLen = Math.max(left.length, right.length);

    for (let i = 0; i < maxLen; i++) {
        var order = compare(left[i], right[i]);
        if (order !== 0) {
            return order;
        }
    }
